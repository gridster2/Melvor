...
/* both of these need to be nulled to prevent errors when
   accessing the page without logging in */
...
async function getMaxHit(monsterId) {
  ...
  if (MONSTERS[monsterId].hasSpecialAttack) {
    for (
      ...
      specialAttackIndex < MONSTERS[monsterId].specialAttackID.length;
      specialAttackIndex++
    ) {
      let specialAttack =
        enemySpecialAttacks[
          MONSTERS[monsterId].specialAttackID[specialAttackIndex]
        ];
      specialAttackChance += specialAttack.chance;
      let setDOTDamage = 0;
      let setDamage = 0;
      if (specialAttack.setDamage != null) setDamage = specialAttack.setDamage;
      if (specialAttack.setDOTDamage != null)
        setDOTDamage = specialAttack.setDOTDamage;
      ...
      for (
        let specialAttackIndex2 = 0;
        specialAttackIndex2 < MONSTERS[monsterId].specialAttackID.length;
        specialAttackIndex2++
      ) {
        let specialAttack2 =
          enemySpecialAttacks[
            MONSTERS[monsterId].specialAttackID[specialAttackIndex2]
          ];
      ...
      if (
        enemyHasStunAttacks &&
        specialAttack.stunDamageMultiplier != null &&
        specialAttack.stunDamageMultiplier > 1
      ) {
  ...
  if (specialAttackChance < 100)
    maxHits[combatData.enemy.attackType].push(
...
async function getMaxHitDungeon(dungeonIndex) {
  ...
  if (dungeon.name === "Into the Mist") {
    ...
    for (let i = 0; i < MONSTERS.length; i++) {
      if (
        getMonsterCombatLevel(i, true) >= 165 &&
        getMonsterCombatLevel(i, true) <= 677 &&
        i !== 134 &&
        i !== 135 &&
        i !== 136
      )
        dungeonMonsters.push(i);
    }
