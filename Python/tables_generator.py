...
auto_eat = [("Auto Eat I", 0.2), ("Auto Eat II", 0.3), ("Auto Eat III", 0.4)]

wasteful_ring_bonus = 0.05

damage_reduction_minimum = 0
damage_reduction_maximum = 81
hitpoints_maximum = 1160
    ...
    for dungeon_index, dungeon in enumerate(dungeon_data):
        for auto_eat_title, auto_eat_threshold in auto_eat:
            ...
            while hitpoints >= hitpoints_minimum + 10:
                                    ...
                                    for enemy_style in ["melee", "ranged", "magic"]:
                                        if "Into the Mist" in dungeon["dungeon"]:
                                            if "(Boss)" in dungeon["dungeon"]:
                                                ...
                                                if (
                                                    monster["id"] == 147
                                                    and player_style != "melee"
                                                ):
                                                    continue
                                                if (
                                                    monster["id"] == 148
                                                    and player_style != "ranged"
                                                ):
                                                    continue
                                                if (
                                                    monster["id"] == 149
                                                    and player_style != "magic"
                                                ):
                                                    continue
                                        ...
                                        if include_wasteful_ring:
                                            modified_auto_eat_threshold += (
                                                wasteful_ring_bonus
                                            )
                                        player_survivable_hit = (
                                            int(
                                                effective_hp
                                                * modified_auto_eat_threshold
                                            )
                                        ...
                                        combat_triangle_key = (
                                            f"{player_style}_vs_{enemy_style}"
                                        )
                                        combat_triangle_key += (
                                            "_hc" if is_hardcore else ""
                                        )
                ...
                if len([1 for i in results_dr if i == "" or "~" in i]) >= len(
                    results_dr
                ):
                    continue
            ...
            while result_check_index < len(results) - 2:
                ...
                while (
                    ...
                    and results[result_check_index][1:] == results[next_index][1:]
                ):
...
def generate_max_hit_table() -> str:
    ...
    for dungeon in dungeon_data:
        ...
        dungeon_monsters = sorted(
            [dungeon_monsters[k] for k in dungeon_monsters], key=lambda x: x["id"]
        )
        max_melee = 0
        max_ranged = 0
        max_magic = 0
    ...
    return (
        "\n".join(
            ["|".join([""] + [str(w) for w in line]) for line in dungeon_display] + [""]
        )
        + "\n\n***\n\n"
    )
...
def rebuild_document() -> None:
    ...
    with open(
        join(docs_dir, "Damage Reduction Tables Intro.md"), "r", encoding="utf-16"
    ) as header_file:
        header_text = header_file.read()

    with open(
        join(docs_dir, "Damage Reduction Tables Mini Intro Insert.md"),
        "r",
        encoding="utf-16",
    ) as intro_file:
    ...
    with open(
        join(docs_dir, "Damage Reduction Tables Mini End Insert.md"),
        "r",
        encoding="utf-16",
    ) as outro_file:
    ...
    with open(
        join(docs_dir, "Damage Reduction Tables for Dungeons.md"),
        "w",
        encoding="utf-16",
    ) as output_file:
    ...
    with open(
        join(docs_dir, "Damage Reduction Tables for Dungeons (Mini).md"),
        "w",
        encoding="utf-16",
    ) as mini_file:
...
def get_dungeon_data() -> list:
    ...
    if len(sorted_dungeon_data) != len(dungeon_data):
        raise Exception(
            f"Mismatch in Dungeon Order ({len(sorted_dungeon_data)}) & Dungeon Data ({len(dungeon_data)})!"
        )
    ...
    sorted_dungeon_data[-2]["monsters"] = [
        m for m in sorted_dungeon_data[-2]["monsters"] if m["id"] not in boss_ids
    ]
    sorted_dungeon_data[-1]["monsters"] = [
        m for m in sorted_dungeon_data[-1]["monsters"] if m["id"] in boss_ids
    ]
